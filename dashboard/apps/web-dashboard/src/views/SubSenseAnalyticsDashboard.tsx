...
import { SmsContactsModal } from "../components/SmsContactsModal";
...
export const SubSenseAnalyticsDashboard: React.FC = () => {
  ...
  const [activeSmsCount, setActiveSmsCount] = useState<number>(2);
  ...
  const handleSimulateDrill = (severity: "critical" | "warning") => {
    ...
    // Dispatch alert to BFF Gateway which automatically fires Termux SMS to all active contacts
    fetchApi("/alerts/simulate", {
      method: "POST",
      body: JSON.stringify({
        site_id: "PANEL7-JHARIA",
        severity,
        explanation: drillAlert.explanation_summary,
      }),
      tenantId: currentTenantId,
      userRole: activeOperator.role,
      userId: activeOperator.userId,
    }).catch((err) => {
      console.warn("[DRILL SMS DISPATCH] BFF simulate error:", err);
    });

    setSystemBanner(
      severity === "critical"
        ? `🚨 CRITICAL COLLAPSE DRILL: Siren activated & automated SMS broadcast sent via Termux to emergency contacts!`
        : `⚠️ WARNING DRILL: Creep warning alert & SMS notification dispatched to safety officers!`
    );
  ...
  return (
    <div className="min-h-screen bg-[#060B14] text-[#E2E8F0] flex flex-col font-sans select-none antialiased">
      ...
      <JudgeDemoConsole
        ...
        onOpenSmsContacts={() => setIsSmsModalOpen(true)}
        activeSmsCount={activeSmsCount}
      />

      {/* Emergency SMS Contacts Management Modal */}
      <SmsContactsModal
        isOpen={isSmsModalOpen}
        onClose={() => setIsSmsModalOpen(false)}
        onContactsUpdated={(count) => setActiveSmsCount(count)}
      />
