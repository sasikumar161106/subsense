...
// Wire packet format for LoRa mesh
...
// Pin for optional relay / gateway indicator LED
...
static void handle_relay_packet(const SubSenseLoraMeshPacket* pkt, size_t wire_len) {
    if (pkt->hop_count >= SUBSENSE_MESH_MAX_HOPS) {
        return; // Drop packet to prevent infinite forwarding loops
    }
    ...
    for (int i = 0; i < SUBSENSE_MESH_DEDUP_CACHE_SIZE; i++) {
        if (s_dedup[i].in_use) {
            if ((now - s_dedup[i].seen_at_ms) > SUBSENSE_MESH_DEDUP_TIMEOUT_MS) {
                ...
                return; // Already forwarded this exact fragment
            }
        }
    }
...
bool subsense_lora_mesh_init(SubSenseMeshRole role, subsense_lora_rx_callback_t rx_callback) {
    ...
    // Initialize underlying SX126x driver (865 MHz, Node Address 0, RSSI enabled)
...
bool subsense_lora_mesh_send(const char* json_payload, size_t len) {
    if (!s_ready || s_role != SUBSENSE_MESH_ROLE_NODE || json_payload == NULL || len == 0) {
        return false;
    }
    if (len >= SUBSENSE_MESH_MAX_PAYLOAD_LEN) {
        return false;
    }
    ...
    pkt.hop_count = 0; // Sensor node is Hop 0
    ...
    for (uint8_t i = 0; i < total_chunks; i++) {
        ...
        if (total_chunks > 1) {
            delay(35); // Allow radio airtime between consecutive fragments
        }
    }
...
void subsense_lora_mesh_loop(void) {
    ...
    // Check for incoming packet from LoRa module
    uint8_t rx_raw[256];
    int16_t rssi_dbm = -70;
    ...
    // Expire stale reassembly slots on Gateway
