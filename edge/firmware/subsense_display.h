 ...
 *
 * Designed for underground harsh mine environments:
 * - Real-time visual confirmation of node health before walking away from installation.
 * - Displays battery %, MPU6050 health, mesh link margin (RSSI), uptime, tilt, and ML status.
 * - Prominent inverted emergency screen when tilt threshold is breached.
 * - Zero external library dependency: built-in lightweight I2C engine using standard Wire.h.
 * - Fail-safe: Automatically detects if OLED hardware is connected; runs safely in headless mode if not.
 */
